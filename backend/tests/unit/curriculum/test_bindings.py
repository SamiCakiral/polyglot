...
def test_bindings_are_typed_frozen_and_credit_roles_are_closed() -> None:
    skill = SkillTargetBinding(
        uid(1), BindingRole.TARGET, ("written_production",), ("produce",), (uid(2),)
    )
    lexicon = LexiconTargetBinding(uid(3), BindingRole.SUPPORT, (uid(4),), (uid(5),))
    grammar = GrammarTargetBinding(
        uid(6), (uid(7),), uid(8), BindingRole.NEW, "request", ("GYM-01",)
    )
    morphology = MorphologyTargetBinding(
        uid(9), "volere.cond.pres", (("person", "1"),), BindingRole.NEW, ("EX-RECALL-06",)
    )
    pronunciation = PronunciationTargetBinding(
        uid(10),
        "e-vs-e-accent",
        uid(11),
        uid(12),
        BindingRole.CONTRAST,
        PronunciationEvaluability.PERCEPTION,
    )
    exercise = ExerciseBinding(
        uid(13), "EX-TRANSFORM-01", ("grammar:request",), uid(14), 2000, 5000
    )
