...
const focusOptions = [
  { id: "grammar", label: "Transformer des phrases", primitive: "EX-TRANSFORM-01", target: "grammar:review" },
  { id: "writing", label: "Écrire dans une situation", primitive: "EX-PROD-01", target: "communication:writing" },
  { id: "vocabulary", label: "Réactiver du vocabulaire", primitive: "EX-RECALL-01", target: "lexicon:due" },
] as const;
...
export function PracticePage() {
  ...
  async function launch() {
    setError("");
    const planId = uuid7();
    const response = await compose.mutateAsync({ profileId, data: { allow_novelty: false, budget_minutes: budget, challenge, modalities: focus.id === "writing" ? ["writing"] : ["reading", "writing"], pedagogical_day: todayIso(), plan_id: planId, primitive_ids: [focus.primitive], snapshot_id: uuid7(), target_refs: [focus.target] } });
